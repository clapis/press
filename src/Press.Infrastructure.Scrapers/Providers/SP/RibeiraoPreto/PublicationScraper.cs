    ...
    IPdfContentExtractor extractor) : IPublicationScraper
{
        ...
        [EnumeratorCancellation] CancellationToken cancellationToken)
    {
        ...
        foreach (var date in dates)
        {
            ...
            {
                using var file = await httpClient.DownloadAsync(link.DownloadUrl, cancellationToken);
    ...
    private async Task<IEnumerable<Link>> ProvideLinksAsync(DateTime date, CancellationToken cancellationToken)
    {
        ...
        using var response = await httpClient.PostAsync(url, JsonContent.Create(new { ID = 9314 },
        ...
        var browserUrl =
            $"https://cespro.com.br/visualizarDiarioOficialLeituraDigital.php?cdMunicipio=9314&dtDiario={date:yyyy-MM-dd}";
